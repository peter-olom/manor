export default async ({ page, artifacts }) => {
  ...
  // Wait additional 3 seconds for any dynamic content
  await page.waitForTimeout(3000);
  
  // Screenshot
  const screenshotPath = `${artifact dir}/manor-inspect.png`;
  await page.screenshot({ path: screenshotPath, fullPage: true });
  
  // Gather data via page.evaluate
  const data = await page.evaluate(() => {
    ...
    // Body innerHTML length and first 500 chars of visible text
    info.bodyInnerHTMLLength = document.body.innerHTML.length ? Actually we need innerHTML of body
    ...
    // Main content area existence and content
    ...
    // Specific CSS classes presence
    const classesToCheck = [
      'workspace-views',
      'workspace-view is-conversation', // note: space means multiple classes
      'pane',
    ...
    // Overlapping/misaligned elements heuristic: check for elements with negative position or overflowing viewport
    ...
    info.overlapIssues = overlapIssues.slice(0,5); // limit to first 5
  ...
  // Attach screenshot to artifacts
  await page.screenshot({ path: `${artifacts.dir}/final.png`, fullPage: true });
