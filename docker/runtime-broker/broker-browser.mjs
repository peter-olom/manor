...
export function createBrokerBrowserController(options) {
  ...
  function formatPlaywrightControlError(error) {
    ...
    const cause = error.cause && typeof error.cause === "object" ? error.cause : {};
    const code = typeof cause.code === "string" ? cause.code : error.name === "AbortError" ? "timeout" : "unavailable";
  ...
  async function callPlaywrightControl(pathname, init = {}) {
    ...
    while (true) {
      ...
      try {
        ...
        if (!isRetryablePlaywrightControlError(error) || attempt >= retryDelaysMs.length) {
