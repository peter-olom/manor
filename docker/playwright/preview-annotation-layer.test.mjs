...
test("preview annotation layer source includes expected toolbar controls and API", () => {
  ...
  assert.match(PREVIEW_ANNOTATION_LAYER_SCRIPT, /viewportRectForMark/);
  assert.match(PREVIEW_ANNOTATION_LAYER_SCRIPT, /documentRectForMark/);
  assert.match(PREVIEW_ANNOTATION_LAYER_SCRIPT, /<rect x="\$\{x\}"/);
  assert.match(PREVIEW_ANNOTATION_LAYER_SCRIPT, /setAttribute\("viewBox"/);
  assert.match(PREVIEW_ANNOTATION_LAYER_SCRIPT, /window\.addEventListener\("scroll", render/);
  ...
  assert.match(PREVIEW_ANNOTATION_LAYER_SCRIPT, /note: mark\.note \|\|/);
  assert.match(PREVIEW_ANNOTATION_LAYER_SCRIPT, /viewport: annotationViewport\(mark\)/);
  assert.match(PREVIEW_ANNOTATION_LAYER_SCRIPT, /setScrollLocked/);
  assert.match(PREVIEW_ANNOTATION_LAYER_SCRIPT, /shouldLockScroll/);
  assert.match(PREVIEW_ANNOTATION_LAYER_SCRIPT, /Boolean\(state\.draft\) \|\| Boolean\(state\.selectionDrag\)/);
  assert.doesNotMatch(PREVIEW_ANNOTATION_LAYER_SCRIPT, /state\.marks\.length > 0 \|\| Boolean\(state\.draft\)/);
  ...
  assert.match(PREVIEW_ANNOTATION_LAYER_SCRIPT, /setMarkDocumentRect/);
  assert.match(PREVIEW_ANNOTATION_LAYER_SCRIPT, /startClientPoint/);
