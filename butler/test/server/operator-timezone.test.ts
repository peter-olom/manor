...
test("zonedWallTimeToUtcMs shifts a spring-forward gap back by one offset duration", () => {
  // Europe/Berlin springs forward 2026-03-29 02:00->03:00 CET->CEST, so 02:30
  // does not exist. The helper shifts it back by one offset to 01:30 CET
  // (00:30 UTC) so a daily run still fires once near the gap instead of throwing.
  const gap = zonedWallTimeToUtcMs("Europe/Berlin", { year: 2026, month0: 2, day: 29, hour: 2, minute: 30, second: 0 });
  assert.equal(gap, Date.UTC(2026, 2, 29, 0, 30));
