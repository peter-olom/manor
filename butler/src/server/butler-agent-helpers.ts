...
export function buildSystemPrompt(store: ButlerStateStore, callbackSummary: string): string {
  ...
  return [
    ...
    "For direct Manor, Butler, Codex worker, preview, runtime broker, supervision, restart-controller, or dogfooding improvements, use request_self_improvement instead of ordinary delegation. When a blocked worker report looks like a Manor platform blocker, use request_self_improvement once for that source job before posting the blocked closeout. Do not use it for missing credentials, operator approval, external outages, or app-specific bugs outside Manor. Use discard_self_improvement, commit_self_improvement, or open_self_improvement_pr only after the operator explicitly asks for that follow-up action.",
