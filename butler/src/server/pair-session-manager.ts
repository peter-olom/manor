...
function pairSystemPrompt(pairId: string): string {
  return [
    ...
    "Work from goals, constraints, live runtime facts, and evidence. Choose and revise the workflow with judgment. Treat prescribed sequences as adaptable plans unless the operator required the exact method or Manor is enforcing a safety, integrity, ownership, approval, or proof boundary.",
    "Ask the operator only when a missing choice materially changes the outcome. Infer ordinary routing and execution decisions from the task, installed skill instructions, available tools, and observed results.",
    "Whenever a tool posts an operator decision card, end the current turn immediately. Do not call more tools or narrate a guessed result. The operator answer arrives in a new turn.",
    ...
    "Repository-backed skill installation belongs to Butler. Butler's executor has writable scratch and read-only repositories and installed skills; it intentionally lacks Worker credential and configuration volumes. Worker has the normal development runtime, persistent user tools and configuration, writable repositories, and read-only installed skills. Infer the right verification actor from those facts and the skill itself. Butler must use a fresh isolated scratch directory for each attempt and prepare a publishable candidate appropriate to the repository's actual skill shape. Butler chooses and runs meaningful validation against that goal without masking failures. Worker-only capabilities remain runtime requirements rather than package omissions. Worker owns independent proof after approved publication, using a representative goal derived from the intended user outcome and evidence appropriate to the capability.",
...
export class PairSessionManager {
  ...
  async getPairDetail(pairId: string, before: number | null, limit: number): Promise<PairDetail | null> {
    ...
    return {
      ...
      workerSupervision: refreshed.worker ? { ...this.options.store.getThreadSupervision(refreshed.worker.threadId) } : null,
      pendingManorRestartRequest: shell.pendingManorRestartRequest,
      reviewRecords: refreshed.worker ? this.options.store.getThread(refreshed.worker.threadId)?.reviewRecords ?? [] : [],
  ...
  updatePairTitle(pairId: string, title: string): PairDetail | null {
    ...
    return {
      ...
      workerSupervision: updated.worker ? { ...this.options.store.getThreadSupervision(updated.worker.threadId) } : null,
      pendingManorRestartRequest: shell?.pendingManorRestartRequest ?? null,
      reviewRecords: updated.worker ? this.options.store.getThread(updated.worker.threadId)?.reviewRecords ?? [] : [],
