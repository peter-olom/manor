...
function validateChatGptAuth(accessToken: string | undefined, refreshToken: string | undefined, credentialRevision: string): ButlerAuthStatus {
  if (!accessToken || !refreshToken) {
    return buildAuthStatus("chatgpt", false, "Stored ChatGPT credentials are incomplete.", credentialRevision);
  }

  if (!getAccountId(accessToken)) {
    return buildAuthStatus("chatgpt", false, "Stored ChatGPT access token is missing the expected account binding.", credentialRevision);
  }
  ...
  return buildAuthStatus("chatgpt", true, null, credentialRevision);
}
...
export async function readButlerAuthStatus(piAuthPath: string): Promise<ButlerAuthStatus> {
  try {
    ...
    if (data["openai-codex"]?.type === "oauth") {
      return validateChatGptAuth(data["openai-codex"].access, data["openai-codex"].refresh, credentialRevision);
    }

    if (data.openai?.type === "api_key" && data.openai?.key) {
      return buildAuthStatus("api", true, null, credentialRevision);
    }
