...
export function buildButlerCodexTools(access: ButlerAgentToolAccess): ButlerCustomTool[] {
  return [
    ...
    access.defineButlerTool({
      ...
      label: "Restart Manor",
      description:
        "Ask the host controller to restart or update Manor. Requires exact operator confirmation because Butler and Codex may be restarted while this request is running.",
      promptSnippet:
        "request_manor_restart: use only when the operator explicitly asks Butler to restart, update, or self-restart Manor. Require exact confirmation. Use mode source for source checkouts, image for GHCR image installs, or auto to detect. The tool schedules the restart and returns a run id; use read_manor_restart_status after Manor comes back.",
      parameters: Type.Object({
        confirmation: Type.Literal("restart Manor"),
      ...
      execute: async (_toolCallId, params) => {
        ...
        const result = await access.hostController.restart({
          confirmation: typedParams.confirmation,
          mode: typedParams.mode ?? "auto",
          target: typedParams.target ?? "current",
          gitRef: typedParams.gitRef?.trim() || null,
          imageTag: typedParams.imageTag?.trim() || null,
          includeDesktop: typedParams.includeDesktop === true,
          build: typedParams.build,
          update: typedParams.update
        });
        return {
          content: [
            {
              type: "text",
              text: [
                `Host controller accepted Manor restart ${result.run.id}.`,
    ...
    access.defineButlerTool({
      ...
      promptSnippet:
        "read_manor_restart_status: use after a self-restart request, or when the operator asks whether a Manor restart/update completed.",
      parameters: Type.Object({}),
      ...
      execute: async () => {
        ...
        const text = run
          ? [
              ...
              .join("\n")
          : `No Manor restart has been recorded. Detected mode: ${status.detectedMode}.`;
        return {
