...
export function buildDelegationDeveloperInstructions(
  ...
  return [
    ...
    "When the job brief includes planner_step, critic_check, taste_note, or operator_question_policy lines, use them as the mission loop: plan from them, execute against them, critique your result with them, and ask only when the policy says the missing choice materially changes the outcome.",
    "When the job brief includes goal_mode=native_goal, start or maintain the Codex goal for the delegated objective if your surface supports it. If it does not, treat goal_recommendation as the compact completion contract.",
    "When the job brief includes sub_agent_role lines, run those sub-agents inside this worker thread for research, role review, or adversarial checks. Return only distilled summaries in the claims JSON; do not paste raw sub-agent transcripts into the supervisor report.",
