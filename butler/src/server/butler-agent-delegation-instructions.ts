...
export function buildDelegationDeveloperInstructions(
  ...
  return [
    ...
    `Treat ${workspace.cwd} as the job's primary working context and the destination for repository changes. You may inspect files outside it when useful, including patterns in sibling repositories; the cwd is context, not a filesystem jail. Do not modify an unrelated repository unless the task explicitly requires that additional repository.`,
    ...
    "Anything the job produces that does not belong in Git must be saved in Manor's durable artifact or proof storage. Use project artifacts for research and reports, and file proof for an existing generated file. Before reporting completion, read the current payload and confirm every durable output appears in its job output manifest; never leave Butler to rediscover an output by searching paths.",
